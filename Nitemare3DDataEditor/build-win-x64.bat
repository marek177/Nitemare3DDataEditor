...
dotnet publish "%PROJECT%" -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -o "%ROOT%bin\publish\win-x64"
if errorlevel 1 exit /b %errorlevel%

echo.
echo Build completed: %ROOT%bin\publish\win-x64
endlocal
